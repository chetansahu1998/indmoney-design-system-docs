...
func (l *dbTouchTargetLoader) LoadScreenTrees(ctx context.Context, versionID string) ([]TouchTargetScreenTree, error) {
	...
	for rows.Next() {
		...
		treeJSON, err := projects.ResolveCanonicalTree(legacy, gz, zstdBlob)
		if err != nil {
...
func (l *dbScreenModeLoader) LoadScreenModesForVersion(ctx context.Context, versionID string) ([]ScreenModeTree, error) {
	...
	rows, err := l.db.QueryContext(ctx,
		`SELECT s.id, m.mode_label, m.explicit_variable_modes_json,
		        COALESCE(t.canonical_tree, ''), t.canonical_tree_gz, t.canonical_tree_zstd
		   FROM screens s
	...
	for rows.Next() {
		...
		var gz, zstdBlob []byte
		if err := rows.Scan(&screenID, &modeLabel, &varJSON, &legacy, &gz, &zstdBlob); err != nil {
			return nil, err
		}
		treeJSON, err := projects.ResolveCanonicalTree(legacy, gz, zstdBlob)
		if err != nil {
...
func (l *dbScreensWithFlowsLoader) LoadScreensWithFlows(ctx context.Context, versionID string) ([]ScreenWithFlow, error) {
	...
	for rows.Next() {
		...
		var legacy string
		var gz, zstdBlob []byte
		if err := rows.Scan(&sw.ScreenID, &sw.FlowID, &legacy, &gz, &zstdBlob); err != nil {
			return nil, err
		}
		tree, err := projects.ResolveCanonicalTree(legacy, gz, zstdBlob)
		if err != nil {
...
func (l *dbFlowGraphLoader) LoadScreensForFlowGraph(ctx context.Context, versionID string) ([]ScreenForFlowGraph, error) {
	...
	rows, err := l.db.QueryContext(ctx,
		`SELECT s.id, s.flow_id, f.file_id, COALESCE(t.canonical_tree, ''), t.canonical_tree_gz, t.canonical_tree_zstd
		   FROM screens s
	...
	for rows.Next() {
		...
		var legacy string
		var gz, zstdBlob []byte
		if err := rows.Scan(&s.ScreenID, &s.FlowID, &s.FileID, &legacy, &gz, &zstdBlob); err != nil {
			return nil, err
		}
		tree, err := projects.ResolveCanonicalTree(legacy, gz, zstdBlob)
		if err != nil {
