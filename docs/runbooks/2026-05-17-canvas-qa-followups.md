...
### Shipped in commit `<follow-up>` (resolves the previously-deferred items)

- **Audit #1 — concurrency race** (was HIGH): InlineSVGMarkup now uses an optimistic-lock UPDATE. `loadCanonicalTreeRaw` returns `(tree, updated_at, err)`; the UPDATE adds `AND updated_at = ?` so a concurrent writer (live Stage 6 commit, parallel Stage 9, ops fix script) cannot be stomped — when rows-affected is 0 the inliner increments `SkippedConcurrent`, logs `svg inline: concurrent writer; skipping screen`, and a future pass re-attempts against the fresher tree. Both the CLI summary (`concurrent_skipped=N`) and Stage 9's post-pass log line surface the count. No more "pause ingestion before running the CLI" contract — the CLI is safe to run concurrent with live traffic.
- **Audit #2 — `svg_markup` skip vs re-exported bytes** (was MEDIUM): added `SVGInlineDeps.ForceRefresh` plus a `--force` CLI flag. Default behavior is unchanged (skip if `svg_markup` already set). When `--force` is passed, the skip is bypassed and on-disk SVG bytes are re-read + re-sanitized + spliced, overwriting stale markup. Header comment at `svg_inliner.go` was rewritten to document the actual contract instead of contradicting it.

### Other audit findings (already shipped previously)
