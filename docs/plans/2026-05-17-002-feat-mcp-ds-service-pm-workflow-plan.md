...
**Milestone**: The autosync → PRD core write loop is wired end-to-end. Designer creates Figma section → autosync → sub_flow row → prd_state skeletons per frame, with PM-authored content preserved across re-runs. MCP and viewer remain — but the persistence layer is complete.
