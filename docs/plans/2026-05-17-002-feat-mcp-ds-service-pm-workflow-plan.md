...
|---|---|---|---|
| 2026-05-17 | `74d0a76` | **U1** — sub_product + sub_flow tables + repo | 973 lines (mig 0036 + subflow.go + subflow_test.go). 17/17 tests pass. `subFlowSlugify` file-local to avoid collision with `makeSlug` at `repository.go:1337`. `FullSlug` is a method on `SubFlow`. |
| 2026-05-17 | `f945292` | **U2** — autosync sub_flow upsert wiring | 586 lines. `UpsertSubFlowFromSection` bridges autosync to U1. Override precedence: admin overrides on `figma_section.{sub_product,sub_flow}_override` (mig 0029) win over `ParseSectionName`. Rename handling NULLs old binding before linking new one (partial unique index on `figma_section_id` enforces 1:1). Insertion: `syncFileDeep` after `UpsertFigmaPagesAndSections`. 10/10 tests; suite green. Audit correction: overrides live on `figma_section` directly, not a separate classification table. |
| 2026-05-17 | `d21af05` | **U4** — PRD typed-stems schema + repo | 2512 lines (mig 0037 + prd.go + prd_test.go). 8 STRICT tables: prd, prd_tab, prd_state (with frame_name + deleted_at; no role_id), prd_state_{acceptance_criterion, edge_case, copy_string, event, a11y_note}, frame_tag. Mirrors `decisions.go` shape (typed Input, sentinel `Err*` per failure, length caps as constants, read-before-tx). `UpsertPRDState` idempotent on `(prd_tab_id, LOWER(TRIM(label)))` ignoring `deleted_at`. `AddEvent` idempotent on `(state, name)`; **no validation** (TODO comment for Mixpanel verb taxonomy). `AttachFrameTag` strict on `(state, node, variant)` with `ErrPRDFrameTagDuplicate`. `LoadPRD` tree-assembly uses stable pointer holders via map (avoids slice-growth invalidation — pattern flagged for U6b). 16/16 tests; suite green. |
| 2026-05-17 | `96e9a6e` | **U5** — ListSectionFrames | 515 lines added to `figma_section_subtree.go` + test. Reads `LoadSectionSubtree` blob (mig 0030, Go-written), filters depth==section.depth+1 + node_type ∈ {FRAME,INSTANCE,COMPONENT}, sorts by (AbsY asc, AbsX asc) stable. Designer names verbatim, no filtering. Empty/missing returns `[]FrameRow{}` not nil/error. **Caveat:** uses read pool; can't see in-flight autosync writes, so U2b reads in-memory `subtreesBySection` map instead. 12/12 tests; suite green. |
| 2026-05-17 | `adf7010` | **U2b** — auto-skeleton from designer-named frames | 1181 lines added. `AutoSkeletonPRDStates` creates one `prd_state` per direct-child FRAME/INSTANCE/COMPONENT, canvas-y order. Markdown-preservation bug **caught and fixed pre-commit**: `ensureSkeletonRow` reads existing row, updates only frame_name/position/deleted_at on matched rows so PM-authored `condition_md`/`design_handling_md`/`fe_handling_md` + stem children survive re-runs. Figma-default regex (`Frame \d+`, `Rectangle \d+`, etc.) skipped at skeleton creation only — MCP `section.frames` still returns them. Soft-delete on frame removal (deleted_at), restore on UpsertPRDState if frame returns. Single-tab v1 ("default" sentinel). Insertion: poller.go::syncFileDeep after `UpsertSubFlowFromSection` returns; reads in-memory `subtreesBySection`. 13/13 tests (incl. RerunPreservesAuthoredMarkdown); suite green. |
| 2026-05-17 | `9848cee` | **U3** — flow_drd.sub_flow_id + sub_flow-keyed accessors | 722 lines (mig 0038 + drd_collab.go + drd_collab_test.go). Adds nullable `sub_flow_id` column on `flow_drd` with partial unique index `WHERE NOT NULL` + lookup covering index. New methods: `CreateDRDForSubFlow` (idempotent on (tenant, sub_flow_id); returns existing flow_id if pre-existing), `LoadYDocStateBySubFlow` (pure read), `PersistYDocSnapshotBySubFlow` (strict UPDATE, not upsert — caller must Create first; ErrNotFound if missing). Characterization-test locks legacy `PersistYDocSnapshot` upsert behavior pre-column-add. Audit correction: codebase has no `ErrFlowDRDNotFound`; used universal `ErrNotFound`. Schema tags new rows `phase-mcp-u3` to distinguish from legacy `phase5`. 13/13 tests; suite green. |
| 2026-05-17 | `400c5a1` | **U3b** — sub_flow prototype URL + canvas lifecycle (KTD-8) | 1208 lines (mig 0039 + subflow_prototype.go + tests + autosync gate change). Adds 4 nullable columns to sub_flow (prototype_url/title/attached_at/superseded_at). `AttachPrototype` HTTPS-only ≤2048 chars, idempotent, publishes `drd.prototype_attached` SSE. `CanvasLifecycle` → "empty"/"proto-only"/"proto-wip"/"design-shipped" via figma_section + figma_page.page_classification join. Autosync gate: `figma_section_id` only links when section's page has `PageClassFinal` (mig 0029 already wired); on transition sets prototype_superseded_at + publishes `figma.design_shipped`. Reused existing PageClassFinal — no new `sub_product.final_design_page_name` column. Broker injected via `SubFlowEventBroker` interface (`*sse.MemoryBroker` satisfies it). poller.go::Config gained `Broker` field; cmd/server/main.go wiring deferred to ship task. 28/28 new + extended tests; suite green. |
| 2026-05-17 | `1f89391` | **U6** — MCP tool registry (3 visible + 16 deep) | 2912 lines, new package `internal/mcp/`. Mirrors RuleRunner interface (Name/Description/InputSchema/Visibility/Invoke) + auditbyslug Deps factory pattern. 3 Visible meta-verbs (`drd.read`, `prd.author`, `section.inspect`) sized for ~1.5k cold token budget; return `next_actions` hints with lazy schemas pointing at the 16 Deep tools wrapping shipped repo methods (subflow.*, section.frames, drd.*, prd.* family, plus section.outline_states stub for U6b). HTTP endpoints: `GET /v1/mcp/tools` (cold catalog) + `POST /v1/mcp/invoke/{name}` (dispatch). New helpers added to projects package: BootstrapDRDForSubFlow (synthetic-project → flow → flow_drd chain for drd.append), LookupFigmaSectionFileKey (sub_flow.figma_section_id → file_key bridge), GetSubProductByID/BySlug. `prd.export` returns markdown but does NOT write to filesystem — bridge/skill writes locally where the user's `~/INDmoney` is meaningful. `RegisterRoutes` defined but not yet called from main.go (deferred to ship task to avoid parallel-session collision). 19/19 tests; suite green. |
| 2026-05-17 | `f983a82` | **U6b** — coverage wall (section.outline_states) + prd_audit | 1590 lines (mig 0040 + prd_audit.go + prd_outline.go + tests + tools_section.go replace stub + meta.go embed wall). New `prd_audit` table (append-only, second-precision `at`); `RecordPRDAudit` + `LatestPRDAuditByState`. `LoadSectionOutline(subFlow) → WallResult` assembles via separate queries + Go-side join (mirrors LoadPRD's stable-pointer-holders pattern): frames via ListSectionFrames blob + all prd_state for sub_flow (including soft-deleted for orphan surface) + per-stem count rollups + word count + LatestPRDAuditByState for last_touched_*. BindingStatus = bound/untagged/orphaned; orphaned rows carry figma_node_id="" for "orphan badge" rendering. section.outline_states stub replaced; sectionInspectResult.Wall replaces basic frames array; `section.inspect` meta-verb embeds the wall + next_actions point at first untagged frame. Audit thread-through in tools_prd.go deferred to ship task — substrate ships first. 15/15 new tests; suite green. |
| 2026-05-17 | `46fdf5c` | **U9b** — universal slug resolver + conventions doc (KTD-6) | 1086 lines (tools_resolve.go + tests + app/api/resolve/[...slug]/route.ts + docs/conventions/sub-product-slug.md). `resolve(slug)` (Deep) accepts 2-segment ({sub_product}/{sub_flow}) or 3-segment ({sub_product}/{sub_flow}/{state}) slugs; returns sub_flow + figma frames + PRD states + DRD/prototype/lifecycle metadata + Mixpanel event names (from prd_state_event) + stub arrays for recent_events/sentry_issues/storybook_paths/jira_components. Stubs ship as `[]` (not nil) so consumers key on the stable shape — per-team adoption populates the arrays as integrations land. Next.js proxy `/api/resolve/[...slug]` forwards to the MCP HTTP endpoint with JWT pass-through. Conventions doc (143 lines) commits {sub_product}/{sub_flow}/{state} as the org-wide identifier shape (kebab-lowercase, slash-joined) with per-surface notes (Figma section name / PRD entity / Mixpanel event prefix / Storybook story path / Sentry tag / JIRA component). 14/14 tests; full suite + tsc green. |
| 2026-05-17 | `b3ccbe9` (ind-suite) | **U7** — local-stdio MCP bridge | 421 lines across `mcp-bridge/{package.json, src/server.ts, README.md, .gitignore}` + a 488 KB bundled `scripts/server.cjs`. Mirrors `src/mcp/server.ts` (ind-mem) shape: `@modelcontextprotocol/sdk` 1.29.0, `StdioServerTransport`, `ListToolsRequestSchema` + `CallToolRequestSchema` handlers. Bridge holds zero state — `tools/list` proxies `GET /v1/mcp/tools`, `tools/call` proxies `POST /v1/mcp/invoke/{name}` with the args body. JWT precedence: `IND_DS_JWT` (inline) > `IND_DS_JWT_PATH` file > `~/.indmoney/jwt`. Failure modes: missing JWT → 401 with hint pointing at `mint-tokens`; ds-service down → "could not reach" error, bridge stays alive; tool error → `{error, detail}` rendered with code+detail. Progressive discovery: `next_actions` rendered as a markdown bullet list with optional `input_hint` JSON; `schema_hint` rendered as a fenced JSON block. Smoke-tested: `initialize` returns server info; `tools/list` against a closed port returns structured MCP error -32603. Lives in `ind-suite` repo at `~/.claude/plugins/ind-suite/mcp-bridge/`. **Registered server name: `indmoney-ds`** — U8 references tools as `mcp__indmoney-ds__drd_read`, `mcp__indmoney-ds__prd_author`, `mcp__indmoney-ds__section_inspect` (dots in tool names become underscores per Claude's MCP namespacing). |

**Milestone**: The autosync → PRD core write loop is wired end-to-end. Designer creates Figma section → autosync → sub_flow row → prd_state skeletons per frame, with PM-authored content preserved across re-runs.
