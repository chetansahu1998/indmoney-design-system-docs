 ...
 *
 * Authored-intent filter (2026-05-13): the structural "same bbox" rule
 * has two false-positive shapes that drown the picker UI (1200+ chips on
 * Tax Centre) AND prune legitimate icon-layer compositions:
 *   1. Overlapping path layers inside an icon — multiple VECTOR/RECTANGLE
 *      leaves drawn at the same coords (stroke + fill + highlight). All
 *      have name "Vector"/"Rectangle"/"Group" (Figma defaults).
 *   2. Auto-generated wrappers (e.g. `clip0_…` clip-path containers paired
 *      with a `Group` sibling) — distinct names but both Figma defaults.
 * Real design-state variants are full INSTANCE/COMPONENT wrappers with
 * designer-renamed labels ("Default", "Hover", "Type=Primary", …). We
 * skip a candidate group when every member's name looks auto-generated
 * (or when every member is a shape leaf), preserving authored variants
 * while suppressing accidental overlaps.
 */
const FIGMA_DEFAULT_NAME_RE = /^(?:vector|rectangle|ellipse|line|polygon|star|group|frame|combined shape|union|subtract|intersect|exclude|mask(?:\s*group)?|path|image|oval)(?:\s+\d+)?$/i;
/**
 * Clip-path wrappers cover every shape Figma auto-generates around a
 * clip-region: `clip0_175_8267`, `clippath-2`, `clip path group`,
 * `clipPath`, `clip`. Designers never name a real state variant with
 * a `clip*` prefix, so anything in this family is safe to treat as
 * auto-generated.
 */
const FIGMA_CLIP_NAME_RE = /^\s*clip(?:\s*-?\s*path)?[\s\-_0-9]*(?:\s*group)?\s*$/i;
...
function isShapeLeaf(node: AnnotatedNode): boolean {
  if (!node.type || !SHAPE_LEAF_TYPES.has(node.type)) return false;
  return !Array.isArray(node.children) || node.children.length === 0;
}

function looksAutoGenerated(name: string | undefined): boolean {
  if (typeof name !== "string") return true;
  const trimmed = name.trim();
  if (trimmed.length === 0) return true;
  if (FIGMA_DEFAULT_NAME_RE.test(trimmed)) return true;
  if (FIGMA_CLIP_NAME_RE.test(trimmed)) return true;
  return false;
}
...
function tagCoPositionedSiblings(
  ...
  for (const [key, members] of groups) {
    ...
    // Skip when the group lacks authored intent: every member is a shape
    // leaf, OR every member's name is a Figma default. Either signal alone
    // is sufficient — designers never leave true state variants unnamed.
    if (members.every(isShapeLeaf)) continue;
    if (members.every((m) => looksAutoGenerated(m.name))) continue;
