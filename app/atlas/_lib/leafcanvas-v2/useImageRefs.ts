...
export function useImageRefs(
  ...
  useEffect(() => {
    ...
    // Post brain-products migration (2026-05-08): the backend
    // `/image-refs` endpoint accepts EITHER a flow UUID or the
    // project slug. The handler resolves slug → primary flow via
    // PrimaryFlowIDForSlug. So whatever leafID we have, fire the
    // request directly — no more client-side repair walk.
    //
    // Pre-fix the hook tried to repair non-UUID leafIDs by walking
    // leavesByFlow looking for `l.flow === slug && isUUID(l.id)`,
    // but `l.flow` is the product taxonomy name (e.g. "onboarding-kyc")
    // and `l.id` is itself the project slug — neither comparison ever
    // matched, so the repair branch was dead code that returned
    // EMPTY_IMAGE_REFS for every slug-shaped leafID. That manifested
    // as flat-grey IMAGE-fill RECTANGLEs on NRI VKYC's passport +
    // bank-statement screens.
